      ...
      <section className="login-form-panel">
        <div className="login-form-wrap">
          <div className="login-form-heading">
            <h2>{isRegister ? 'Create your account' : 'Sign in to DataMate'}</h2>
            <p>{isRegister ? 'Set up your credentials to get started.' : 'Use your workspace credentials to continue.'}</p>
          </div>
          <form className="login-form" onSubmit={submit} noValidate>
            {isRegister && (
              <label><span>Full name</span><div className="login-field"><User size={17} /><input type="text" value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" placeholder="Your full name" /></div></label>
            )}
            ...
            <button className="login-submit" type="submit" disabled={isSubmitting}>{isSubmitting ? (isRegister ? 'Creating account…' : 'Signing in…') : <>{isRegister ? 'Create account' : 'Sign in'} <ArrowRight size={17} /></>}</button>
          </form>
